...
const validateAddVideoData = [
  ...
  body("title", "title must have at least 1 - 30 characters").isLength({
    min: 1,
    max: 30,
  }),
  body("description", "description is required").not().isEmpty(),
  body("description", "description must have at least 1 character").isLength({
    min: 1,
  }),
  body("duration", "duration is required").not().isEmpty(),
  body("duration", "duration must be at least 1 second long").isInt({ min: 1 }),
  body("url", "url is required").not().isEmpty(),
  body("url", "url must be valid").isURL(),
  body("uploaded_by_user", "uploaded_by_user is required").not().isEmpty(),
  body("uploaded_by_user", "uploaded_by_user must be valid").isLength({
    min: 1,
    max: 16,
  }),
];
